...
import CommunityQueryBox from '../../components/CommunityQueryBox'
...
export default function ResultPage() {
  ...
  useEffect(() => {
    ...
    const fetchForecast = async () => {
      ...
      const apiBase = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://127.0.0.1:8000'
      const appliance = pageState.input.appliance_type || pageState.input.appliance || ''
      const city = pageState.input.city || ''
      
      try {
        const res = await fetch(`${apiBase}/api/forecast/trend?appliance=${encodeURIComponent(appliance)}&city=${encodeURIComponent(city)}`)
        const json = await res.json()
        if (json && json.trend) {
          setTrendData(json.trend)
          setForecastText(json.forecast)
        }
      } catch (e) {
        console.error('Error fetching price forecast trend:', e)
      } finally {
    ...
    <>
      ...
      <main className="result-main">
        <div className="container result-container">
          ...
          <CommunityQueryBox 
            currentCity={pageState.input?.city || ''} 
            currentAppliance={pageState.input?.appliance_type || pageState.input?.appliance || ''} 
          />
          ...
          <div className="result-grid">
            ...
            <PriceForecast 
              ...
              appliance={pageState.input?.appliance_type || pageState.input?.appliance || ''} 
              city={pageState.input?.city || ''} 
            ...
            {(activeVerdict === 'high' || activeVerdict === 'suspicious') && (
              <div className="result-card glass-panel dispute-helper-box" style={{
                padding: '24px',
